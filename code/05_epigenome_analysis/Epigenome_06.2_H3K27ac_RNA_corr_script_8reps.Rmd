...
FIMO_CTCF_plus_bed <- FIMO_CTCF %>% dplyr::filter(strand=="+") %>% dplyr::select("chrom"="sequence_name", "start", "end"="stop", "strand")
FIMO_CTCF_minus_bed <- FIMO_CTCF %>% dplyr::filter(strand=="-") %>% dplyr::select("chrom"="sequence_name", "start", "end"="stop", "strand")
...
```{r}
CDvsHFD_H3K27ac_Diffbind <- readRDS("results/Epigenome_analysis/Diffbind_all_annotated_DB_peaks.rds") $ K27_CDvsHFD %>% 
...
H3K27ac_CTCF_intersect_log2FC <- inner_join(H3K27ac_CTCF_intersect,CDvsHFD_H3K27ac_Diffbind, by="loc_ID") %>% dplyr::select(4:8,"log2FC"="V9",  "FDR" ="V11")
