...
# Use BEDOPS suite to determine the closest TSS to the enhancer sites. Run this in terminal, it doesn't matter in which folder you are if the correct paths are set. Requires BEDOPS and bedtools.
```{bash, eval=F}
...
closest <- read.delim("5_Correlation_GRE_identification/5.1_bedops_closest/220629_origin_closest1_left_closest_1_2_right.bed", header=F)
...
```{r}
getwd()
TPM <- read.delim("5_Correlation_GRE_identification/5.1_bedops_closest/220609_TPM_ordered_df.txt")
TPM <- TPM %>% dplyr::select(1,2, 11,14,15, 16, 24,25, 30,31)
# We will name the mice according to their original mouse number instead of replicate number. 
...
write.table(K27_GE_long_group, "220704_K27_GE_corr_before_filtering.txt", quote=F, row.names=F, sep="\t")
#load the reverted gene sets
revUP <- read.delim("/Users/christian.som/Dropbox/MAFLD_Figure_drafts/Results/DEGs/filtered/220204_up_reverted_Genes_symbol_ensembl_CS.txt", header=T)
revDOWN <- read.delim("/Users/christian.som/Dropbox/MAFLD_Figure_drafts/Results/DEGs/filtered/220204_down_reverted_Genes_symbol_ensembl_CS.txt", header=T)
revALL <- read.delim("/Users/christian.som/Dropbox/MAFLD_Figure_drafts/Results/DEGs/filtered/220204_all_reverted_Genes_symbol_ensembl_CS.txt", header=T)
nrow(revALL)
K27_GE_long_rev_insect <-  K27_GE_long_group %>% filter(ensembl_gene_id %in% revALL$ensembl_gene_id)
length(unique(K27_GE_long_rev_insect$ensembl_gene_id))

write.table(K27_GE_long_rev_insect, "5_Correlation_GRE_identification/220701_corr_table_FINDOUTWHATHAPPENED.txt", quote=F, sep="\t")
...
ggplot(intersect_pathways_symbols) +
  ...
  ggsave("5_Correlation_GRE_identification/220703_hist_GSEA_terms.pdf", height=10, width=15, units="cm")
```
...
View(as.data.frame(GO_BP))
  ...
  ggplot(plot_me_ordered, aes(x=factor(Description, levels=name_order), fill=p.adjust, y=Count)) +
    ...
    ggtitle("") + 
    xlab("") + 
    ggsave("220704_45_CTCF_revDEG_GO_BP.pdf", width = 20, height = 10, units = "cm")
```
