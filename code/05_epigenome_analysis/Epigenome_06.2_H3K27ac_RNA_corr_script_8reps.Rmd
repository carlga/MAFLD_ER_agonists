...
# Row 738 has an NA in several columns (including V8), remove this one as it causes issues downstream. This lowers the number of potential pairs.
closest2 <- closest %>%
...
write.table(K27_GE_joined, "Supplemental_tables/SupplementalTable_initial_ESEGs.txt", quote=F, row.names=F, sep="\t")
```
...
#write.table(K27_GE_long_group, "Supplemental_tables/SupplementalTable_Initial_ESEG_genes_corr.txt", quote=F, row.names=F, sep="\t")

# Filter the ESEGs for a | pearson correlation |  > 0.75
...
#write.table(K27_GE_long_group_plot_pearson, "Supplemental_tables/SupplementalTable_step2_ESEG_genes_corr_0.75pearson.txt", quote=F, row.names=F, sep="\t")
...
length(unique(K27_GE_long_rev_insect$ensembl_gene_id)) # 67 unique genes remain after filtering for reverted gene set.
nrow(K27_GE_long_rev_insect)/8 # 131 enhancer gene pairs remain after filtering

#write.table(K27_GE_long_rev_insect, "Supplemental_tables/SupplementalTable_step3_ESEG_genes_corr_0.75pearson_reverted.txt", quote=F, row.names=F, sep="\t")
```
...
length(unique(H3K27ac_CTCF_intersect$loc_ID)) # Some enhancers (= loc_IDs) are duplicated, 67 unique ones.
...
#write.table(K27_GE_long_group_plot_filt, "Supplemental_tables/SupplementalTable_step4_ESEG_genes_corr_rev_CTCF.txt", row.names = F,  quote = F, sep="\t")
...
# 68 total combinations of location IDs and genes, one enhancer goes for two genes.
...
library(ggpubr)
  ggscatter(K27_GE_long_group_plot_filt, x = "H3K27ac", y="Gene_expression", add = "reg.line",
            ...
            xlab="H3K27ac",
            ylab="TPM",
            palette=c("#88CCEE","#88CCEE", "#DDCC77", "#DDCC77", "#AA4499","#AA4499","#332288","#332288")) +
...
write.table(as.data.frame(GO_BP), "Supplemental_tables/SupplementalTable_GO_BP_ESEG_genes.txt", row.names = F, sep="\t", quote=F)
...
#ggsave("results/GO_plot_FigS7D.pdf", width = 18, height=12, units="cm")
```
