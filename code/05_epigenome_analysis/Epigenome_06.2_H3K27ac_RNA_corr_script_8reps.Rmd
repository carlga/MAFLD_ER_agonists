...
# 46 unique genes that underlie potential enhancer-mediated estrogen-dependent regulation
```
...
length(unique(K27_GE_long_group_plot_filt$symbol)) # 46 unique genes
nrow(K27_GE_long_group_plot_filt)/8 # 70 enhancer - gene pairs
length(unique(K27_GE_long_group_plot_filt$loc_ID)) # 68 unique enhancers

# 68 total combinations of location IDs and genes, one enhancer goes for to genes.

write.table(K27_GE_long_group_plot_filt, "results/Epigenome_analysis/corr_46genes_69enh_toPlot.txt", row.names = F,  quote = F, sep="\t")
```
...
reactome_pathways.2 <- as.data.frame(do.call(cbind, test))
