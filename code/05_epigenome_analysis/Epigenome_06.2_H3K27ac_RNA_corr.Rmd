...
counts_enha <- counts_enha <- read.delim("231130_K4me3_H3K27ac_new_analysis/Results/231130_reads_in_peaks/DAc_enhancers_2181_H3K27ac.clean.readCount", header=T) %>% tibble::column_to_rownames("Geneid") %>%
...
nrow(K27_GE_long_group_plot_pearson)/18  # XXX enhancer gene pairs remain.
...
nrow(K27_GE_long_rev_insect)/18 # XXXX enhancer gene pairs remain after filtering
...
length(unique(H3K27ac_CTCF_intersect$loc_ID)) # Some enhancers (= loc_IDs) are duplicated, XXXX unique ones.
unique_symbols <- unique(H3K27ac_CTCF_intersect$symbol)
length(unique_symbols)
# XXXX unique genes that underlie potential enhancer-mediated estrogen-dependent regulation
```
...
length(unique(K27_GE_long_group_plot_filt$symbol)) # XXX unique genes
nrow(K27_GE_long_group_plot_filt)/18 # XXXX enhancer - gene pairs
length(unique(K27_GE_long_group_plot_filt$loc_ID)) # XXXX unique enhancers
...
library(ggpubr)
  ggscatter(K27_GE_long_group_plot_filt, x = "H3K27ac", y="Gene_expression", add = "reg.line",
            ...
            xlab="H3K27ac",
            ylab="TPM",
             palette=c("#88CCEE",  "#332288", "#DDCC77", "#CC6677", "#AA4499",  "#882255")) +
    stat_cor(aes(label = ..r.label..),method="pearson",cor.coef.name = "r", p.digits=0, size=5, p.accuracy = 0.001) +
    theme_bw() + 
