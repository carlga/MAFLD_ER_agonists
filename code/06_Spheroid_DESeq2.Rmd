...
Condition_inhibitors <- c(rep("plusFFA",3), rep("TEADap",2), rep("TEADsf",3))
...
write.table(export_TPM_normTables, "results/spheroid_TPM_norm_counts.txt", sep="\t", quote=F)
```
...
DESeq2_DEGs_inhibitors$unfilt <- list(
  TEADap_vs_control = results(ds_data_inhibitors.filt, contrast = c('Condition', 'TEADap', 'plusFFA')),
  TEADsf_vs_control = results(ds_data_inhibitors.filt, contrast = c('Condition', 'TEADsf', 'plusFFA')))
