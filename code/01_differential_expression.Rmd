...
# comparisons
DESeq2_DEGs <- list(
  CDfVsCDm = results(ds_data, contrast = c('condition', 'CDf', 'CDm')),
  HFDfVsHFDm = results(ds_data, contrast = c('condition', 'HFDf', 'HFDm')),
...
# comparisons
edgeR_DEGs <- list(
  CDfVsCDm = glmLRT(fit_dge, contrast = makeContrasts(groupsCDf-groupsCDm, levels = design)),
  HFDfVsHFDm = glmLRT(fit_dge, contrast = makeContrasts(groupsHFDf-groupsHFDm, levels = design)),
...
saveRDS(DEGs, file = 'results/bulkRNAseq_mmus_DEGs.rds')
